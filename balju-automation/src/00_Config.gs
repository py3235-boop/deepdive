...
/**
 * 이 발주서가 **추가**인지 **수정**인지 가린다.
 *
 * 같은 품목·같은 납기일로 다시 오는 경우, 데이터만으로는 두 가지를 구분할 수 없다.
 *   "그 줄의 수량을 9 → 12 로 바꿔주세요"   → 수정 (이전 행을 대체해야 한다)
 *   "그 날짜에 2개 더 넣어주세요"            → 추가 (이전 행을 그대로 두고 붙여야 한다)
 *
 * 유일한 단서는 사람이 쓴 말이다. 제목·본문·파일명에서 찾는다.
 *
 * @param {string[]} 텍스트들 제목·본문·파일명 등
 * @return {'추가'|'수정'|null}
 */
function 발주종류판정(텍스트들) {
  const 합친것 = (텍스트들 || []).map(키정규화).join(' ');

  // `추가` 를 먼저 본다 — "추가 발주 수정본" 같은 경우 추가가 더 중요한 신호다
  if (합친것.indexOf('추가') >= 0) return '추가';
  if (/수정|변경|정정|재발주/.test(합친것)) return '수정';
  return null;
}
