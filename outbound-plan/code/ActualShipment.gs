 ...
 * 날짜에 다시 배분한다. 대회구현가이드 §6/§11-5 방식 — 74340 파일만은 헤더 텍스트가 아니라 고정
 * 컬럼 위치로 읽는다(더미 생성기와 컬럼 순서를 맞춰뒀다는 전제).
 *
...
const ACTUAL_COL_ = {
  ...
  shipDate: 8, // I열: 출고일자
  customer: 10, // K열: 납품처 ("고객사A" 형태 → 마지막 알파벳만 추출해 "A사"로 변환)
};
...
function _applyActualShipment_() {
  ...
  saved.rows.forEach(row => {
    ...
    updatedRows.push({ code: row.code, spec: row.spec, vendor: row.vendor, dateMap: newDateMap, changed: false });
  ...
  vendorOrder.forEach(vendor => {
    ...
    if (bucketWeekday !== undefined) {
      ...
      // 트럭버킷이 아닌 타입은 여기서 직접 vendorDateLoad를 트럭대수(kg ÷ TRUCK_KG, 올림)로 갱신한다.
      items.forEach(it => {
...
function _loadActualShipments_() {
  ...
  values.slice(1).forEach(row => {
    ...
    if (!m) return;
    const vendor = m[1].toUpperCase() + '사';
